if CLIENT then
    --[[this certainly needs to be optimized--]]
    function ConvexLensImaging(lens)
        ...
        if tar_front:IsValid() == true then
            ...
            --[[try not to create any new entities--]]
            if tar_front:GetClass() == "prop_physics" then
                local rendermode_old_prop0 = tar_front:GetRenderMode()
                tar_front:SetRenderMode(RENDERMODE_NONE)
                tar_front:DrawModel()

                local data0 = render.Capture({
                    format = "png",
                    quality = imagingquality,
                    x = 0,
                    y = 0,
                    w = w,
                    h = h,
            ...
            --[[this can't take photos of the player in singleplayer--]]
            if tar_front:IsPlayer() == true and tar_front ~= LocalPlayer() then
                local rendermode_old_player0 = tar_front:GetRenderMode()
                tar_front:SetRenderMode(RENDERMODE_NONE)
                tar_front:DrawModel()

                local data1 = render.Capture({
                    format = "png",
                    quality = imagingquality,
                    x = 0,
                    y = 0,
        ...
        --[[delete ing the unnecessary files on lens remove--]]
        lens:CallOnRemove("Optics_DeleteImagingFilesOnLensRemove", function()
            for k, v in pairs(imagestable) do
