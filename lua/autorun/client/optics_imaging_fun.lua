   ...
   function ConcaveLensImaging(lens)
      ...
      if tar:IsValid() == true then
        ...
        cam.Start3D(pos_direct,angles:Right():Angle(),100)
          ...
          if tar:IsPlayer() == true then  // this can't take photos of the player in singleplayer
             tar:SetRenderMode(RENDERMODE_NONE)
             tar:SetRenderAngles(angles)
             tar:DrawModel()
             print(cam.GetModelMatrix())
             local data2 = render.Capture({ format = "png", quality = 50, x = 0, y = 0, w = w, h = h, alpha = true })
